...
export function widgetStream(parts: TemplateStringsArray, ...includes: IWidgetStream[]): IWidgetStream {
  const streamer = parts.reduce((streamer, part, index) => {
    if (index === 0) {
      return streamer.write(part);
    } else {
      return streamer.pipe(includes[index - 1]).write(part);
    }
  }, new WidgetStream());
