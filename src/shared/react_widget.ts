...
export class ClientReactWidget<TProps, TComponent extends TReactComponent<TProps>> implements IClientWidget<TProps> {
  ...
  public attach(element: Element, props: TProps): IAttachedWidget<TProps> {
    const update = (props: TProps) => {
      hydrate(React.createElement(this.component, props), element);
    };
    const detach = () => {
    ...
    return {
      subscribe: (dataCallback, endCallback) => {
        if (complete) {
          ...
          complete = true;
          result = renderToString(React.createElement(this.component, props));
          dataCallback(result);
          endCallback();
        } else if (!complete && this.useStream) {
          if (started) {
            ...
            const stream = renderToNodeStream(React.createElement(this.component, props));
